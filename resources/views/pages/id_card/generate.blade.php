            ...
            <div class="col-lg-12">
                <div class="row tab-content">
                    <div id="list-view" class="tab-pane fade active show col-lg-12">
                        ...
                        <div class="card">
                            <div class="card-body">
                                @include('pages.id_card.partials._filter')

                                <form action="{{route('idcard.generate_file')}}" method="post" target="_blank"
                                    class="mb-3 text-right">
                                    @csrf
                                    <input type="hidden" id="idcardId" name="idcard_id"
                                        value="{{@$filter['idcardId']}}">
                                    <input type="hidden" id="paperSize" name="paper_size"
                                        value="{{@$filter['paperSize']}}">
...
<script>
    $(function(){
        ...
        $(document).on('change', '#checkAll, input[name="student_id[]"]', function(){
            const studentIds = $('input[name="student_id[]"]:checked').map((_,idInput) => $(idInput).val()).get();
            $('#studentIds').val(studentIds.join(','));
        });

        $('#generateButton').click(function(e){
            const isValid = $('#idcardId').val() && $('#paperSize').val() && $('#studentIds').val();
            if(!isValid){
                alert('Please select idcard design & student');
                return;
            }
