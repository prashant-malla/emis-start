                                ...
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table id="example3" class="display" style="min-width: 750px">
                                            ...
                                            <tbody>
                                            @foreach(\App\Models\Eclass::all() as $eclass)
                                                <tr>
                                                    <td>{{$eclass->class_name}}</td>
                                                    <td>
                                                        <div class="d-flex">
                                                            <a href="{{route('class.edit',$eclass['id']) }} " class='btn btn-sm btn-primary m-1'>
                                                                <i class="la la-pencil"></i></a>
                                                            <form method="POST" action=" {{route('class.destroy',$eclass->id)}}" onsubmit="return confirm('Are you sure?')">
                                                                @method('delete')
                                                                @csrf
