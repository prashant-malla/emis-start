                                        ...
                                        <table id="example3" class="display" style="min-width: 750px">
                                            ...
                                            @foreach(\App\Models\Session::all() as $session)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="d-flex">
                                                            <a href="{{route('session.edit',$session['id']) }} " class='btn btn-sm btn-primary m-1'>
                                                                <i class="la la-pencil"></i></a>
                                                            <form method="POST" action=" {{route('session.destroy',$session->id)}}" onsubmit="return confirm('Are you sure?')">
                                                                @method('delete')
                                                                @csrf
