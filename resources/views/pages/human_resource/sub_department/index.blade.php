                        ...
                        <div id="list-view" class="tab-pane fade active show col-lg-12">
                            ...
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <a href="{{route('sub_department.create')}}" class="btn btn-primary">+ Add new</a>
                                </div>
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table id="example3" class="display" style="min-width: 845px">
                                            ...
                                            @foreach($subDepartments as $key =>$subDepartment)
                                                <tr>
                                                    <td>{{$subDepartment->department->department}}</td>
                                                        ...
                                                        <div class="d-flex">
                                                            ...
                                                            @if(\Illuminate\Support\Facades\Auth::guard('staff')->check())
                                                                @if(auth()->guard('staff')->user()->role == 'admin')
                                                                    {{route('admin.sub_department.edit', $subDepartment)}}
                                                                @endif
                                                            @elseif(\Illuminate\Support\Facades\Auth::user()->role == 'superadmin')
                                                                {{route('sub_department.edit', $subDepartment)}}
                                                            @endif
                                                            ...
                                                            @if(\Illuminate\Support\Facades\Auth::guard('staff')->check())
                                                                @if(auth()->guard('staff')->user()->role->name == 'Admin')
                                                                    {{route('admin.sub_department.destroy',$subDepartment)}}
                                                                @endif
                                                            @elseif(\Illuminate\Support\Facades\Auth::user()->role == 'superadmin')
                                                                {{route('sub_department.destroy',$subDepartment)}}
                                                            @endif
                                                                " method="post"
