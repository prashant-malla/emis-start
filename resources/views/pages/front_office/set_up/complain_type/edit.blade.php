                ...
                <div class="col-xl-12 col-xxl-12 col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title">Edit Feedback Type</h5>
                        </div>
                        <div class="card-body">
                            ...
                            <form action="
                                 {{-- @if(\Illuminate\Support\Facades\Auth::guard('staff')->check())
                                     @if(auth()->guard('staff')->user()->role->name == 'Admin')
                                        {{route('admin.complain-type.update')}}
                                     @elseif(auth()->guard('staff')->user()->role->name == 'Receptionist')
                                        {{route('receptionist.complain-type.update')}}
                                     @elseif(auth()->guard('staff')->user()->role->name == 'Accountant')
                                        {{route('accountant.complain-type.update')}}
                                     @endif
                                 @elseif(\Illuminate\Support\Facades\Auth::user()->role == 'SuperAdmin')
                                    {{route('complain-type.update')}}
                                 @endif --}}
                                 {{route('complain-type.update')}}
                                " method="POST">
                                <input type="hidden" name='id' value={{$complainType['id']}}>
                                @csrf
                                <div class="row">
                                    <div class="col-lg-6 col-md-6 col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="complain_type" value="{{$complainType->complain_type}}">
